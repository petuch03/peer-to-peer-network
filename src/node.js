...
let name = randomItem(['Gorgeous', 'Elegant', 'Fantastic', 'Smart']) + ' ' + randomItem(['pine', 'oak', 'spruce']) + ' from ' + randomItem(['Paris', 'Berlin', 'Belgrade', 'Ljubljana']);
...
node.listen(port, () => {
    ...
    process.stdin.on('data', (data) => {
        ...
        if (text.startsWith('connect')) {
            ...
            console.log(`Disconnecting from ${ip} at ${Number(port)}...`);
            node.disconnect(ip, Number(port), () => {
                console.log(`Connection to ${ip} dropped.`);
            });
        } else {
